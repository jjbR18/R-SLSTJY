 ...
 * @text 敌方SV战斗图
 * @desc 如果使用动画SV敌人，如果没有图标被使用，绘制他们的战士？这可能会变得滞后。 NO - false  YES - true
 * @default false
 *
 * @help
 * ============================================================================
 ...
 * ============================================================================
 *
 * OK,其实我很乐意给这类战斗插件做翻译,CTB系统也是一个很经典的回合战斗系统.
 * 你一定玩过空轨、碧轨、零轨对吧，那你一定对这种战斗系统不陌生,类似于ATB系
 * 统,CTB系统也有一个行动槽,但是与ATB不同,他是所有角色统一的一个排位槽,角色
 * 根据自身敏捷进行排位槽积攒排位值,最先攒满的角色进行行动,行动中角色可以使
 * 用立即施放,或需咏唱一段时间的技能，在选择咏唱技能后,其他角色的行动槽将继
 * 续增长,到达顶点后开始行动.
 *
 * 类似与ATB插件,本插件也将依靠Battle Engine Core核心插件,安装时务必位于核心
 * 插件下方同时要启用CTB战斗系统,你也需要在核心插件的Default System选项中
 * 将键值修改为CTB即可
 *
 * ============================================================================
 ...
 *
 * 插件命令:
 *   setBattleSys CTB      设置为CTB系统
 *   setBattleSys DTB      设置为DTB系统.
 *
 ...
 *   <Mage CTB Icon: x>
 *   <Priest CTB Icon: x>
 *   如果你要使用职业名称来定义头像也是可以的,但写法略有不同,把上面class x直接
 *   替换为职业名称即可.高ID优先
 *
 * 技能和道具注释命令:
 *   <CTB Help>
 ...
 *   <CTB Order: +x>
 *   <CTB Order: -x>
 *   使用这个命令按照x参数改变当前目标的综合排序,+x会使目标需要等待更长的时间.
 *   这个命令的效果微小,并且只会持续到当前回合的循环结束
 *   注意：如果你的命令针对了多个目标,每个目标的回合变化将会分别计算
 *   其实就是按照x排序啦,比如你写+1就是把目标的行动推后1位,由于只
 *   在当前回合的循环轮生效,所以+100和+1000的效果都是推到最后,但是+1000肯定能
 *   够要比+100更靠后.如果x相同,那么将会根据原来的角色顺序排序.这个命令我认为
 *   作者其实主要想用来微调某个角色的行动顺序的功能,因为靠speed参数来调整需要
 *   的计算量比较大,而且很不稳定.而这个+1 +2 -1 -2的排序功能设定起来就相对容易
 *   多了.
 *   
 *   individually at a time.
 *
 *   <After CTB: x>
 *   <After CTB: x%>  在行动结束后,将排位槽的值设置为x或x%
