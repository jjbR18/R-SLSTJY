 ...
 *
 * 這是一種鬧鐘功能，可以通過指定時間而不是經過時間來進行操作。
 * 請將[年月時分]以「YYYYMMDDHHMM」的形式指定。
 * 可以使用計時器命令取消（需使用有指定「鬧鐘名稱」的插件命令）。
 * C_SET_SWITCH_ALARM [年月時分] [開關ID]
 * C_SET_SELF_SWITCH_ALARM [年月時分] [自開關]
 * C_SET_SWITCH_NAMED_ALARM [鬧鐘名稱] [年月時分] [開關ID]
 * C_SET_SELF_SWITCH_NAMED_ALARM [鬧鐘名稱] [年月時分] [自開關]
 *
 * 指定範例（遊戲內時間 2019/10/22 15:00 經過時打開自開關 [B]）
...
(function() {
    ...
    var settings = {
        /* timeZone:時間帶 */
        timeZone: [
            /* name:時間帶名稱 start:開始時間 end:結束時間 timeId:時間帶ID */
            {name: '點名', start: 7, end: 7, timeId: 1},
            {name: '自由時間', start: 8, end: 13, timeId: 2},
            {name: '工作時間', start: 14, end: 15, timeId: 3},
            {name: '洗澡時間', start: 16, end: 16, timeId: 4},
            {name: '自由時間', start: 17, end: 21, timeId: 5},
            {name: '晚間點名', start: 22, end: 22, timeId: 6},
			{name: '深夜', start: 23, end: 6, timeId: 0}
        ],
        /* timeTone:時間帶色調 */
        timeTone: [
            /* timeId:時間帶ID value:色調[紅(-255...255),綠(-255...255),藍(-255...255),灰(0...255)] */
            {timeId: 0, value: [-102, -102, -68, 102]},
            {timeId: 1, value: [-48, -48, 0, 0]},
            {timeId: 2, value: [0, 0, 0, 0]},
            {timeId: 3, value: [34, 34, 34, 0]},
            {timeId: 4, value: [68, -34, -34, 0]},
            {timeId: 5, value: [-68, -68, 0, 68]},
			{timeId: 6, value: [-68, -68, 0, 68]}
        ]
    };
    //=============================================================================
    // 使用者設定區域 - 終了 -
    ...
    Game_Chronus.prototype.convertDateFormatText = function(format) {
        ...
        format = format.replace(/AM/gi, function() {
            return Math.floor(this.getHour() / 12) === 0 ?
                $gameSystem.isJapanese() ? '上午' : '上午' : //修正「上午」名稱
                $gameSystem.isJapanese() ? '下午' : '下午'; //修正「下午」名稱
        }.bind(this));
