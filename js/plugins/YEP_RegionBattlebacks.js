 ...
 * @param Default 1
 * @text 默认值1
 * @desc bullettback1默认使用的文件名。
 * Default: Grassland
 * @default Grassland
 *
 * @param Default 2
 * @text 默认值2
 * @desc bullettback2默认使用的文件名。
 * Default: Grassland
 * @default Grassland
 *
 * @param ----------
 ...
 * 面parameters配置中,Battleback1表示背景图下层，
 * 即img\battlebacks1中的文件Battleback2表示背景图上层，
 * 即img\battlebacks2中的文件，不再单独解释。
 * paremeters配置主要让你修改默认地图战斗背景使用，
 * 如果你想使用region区块标记不同的战斗背景,请使用地图配置界面的
 * 注释命令。
 * 使用世界地图时，你会发现自己并不能使用region区块来控制战斗背景。
 * 当然，本插件还能有效避免因为其他地图上战斗背景名称
 * 不匹配和加载失败引起的游戏崩溃。
 * 本插件可以允许你改变默认地形的默认战斗背景，同时也可以让你
 * 用地图注释命令定义使用region区块定义的特别战斗背景
 *
 * ============================================================================
 * Notetags
 * ============================================================================
 ...
 *   很简单x为region区块的ID号,filename为背景图片名称。
 *   注意这里只需要用文件名，不需要路径，
 *   你的所有战斗背景都应该放在游戏工程的原始目录下面。
 *
 *   For example:
 *
