 ...
 * @desc 如果不想显示,填写为空。
 * @default 拥有配方数量/已知配方总数量:
 *
 * @param Crafted Items
 * @text 道具配方数量命令文本
 * @parent ---Status Window---
 * @desc 如果不想显示,填写为空。
 * @default 已知道具配方数量/道具配方总数量:
 *
 * @param Crafted Weapons
 * @text 武器配方数量命令文本
 * @parent ---Status Window---
 * @desc 如果不想显示,填写为空。
 * @default 已知武器配方数量/武器配方总数量:
 *
 * @param Crafted Armors
 * @text 防具配方数量命令文本
 * @parent ---Status Window---
 * @desc 如果不想显示,填写为空。
 * @default 已知防具配方数量/防具配方总数量:
 *
 * @param ---List Window---
 * @text 列表窗口
 * @default
 ...
 * @text 代替尚未合成的物品名称
 * @parent ---List Window---
 * @desc "隐藏尚未合成的物品名称"为true,用于替代尚未合成的物品名称。
 * @default ????
 *
 * @param Mask Italic
 * @text 使替代名称文本斜体显示
 * @parent ---List Window---
 * @type boolean
 * @desc "隐藏尚未合成的物品名称"为true,使替代名称文本斜体显示。
 * @default true
 *
 * @param Mask Help Text
 * @text 隐藏物品帮助窗口文本
 * @parent ---List Window---
 * @desc "隐藏尚未合成的物品名称"为true,如果物品被隐藏,帮助窗口显示的文本。
 * @default 此物品未知。
 *
 ...
 *
 * @help
 * 插件更新:yanfly.moe/plugins/en/YEP_ItemSynthesis.js
 * 〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓
 * 道具/武器/防具备注:
 * ＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
 * 将需要合成的物品备注到配方里:
 * <Item Recipe: x> / <Item Recipe: x, x, x> / <Item Recipe: x to y>
 * <Weapon Recipe: x> / <Weapon Recipe: x, x, x> / <Weapon Recipe: x to y>
 * <Armor Recipe: x> / <Armor Recipe: x, x, x> / <Armor Recipe: x to y>
 * ＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
 * 合成该物品需要的材料:
 ...
 * gold: x
 * </Synthesis Ingredients>
 * ※注意:无法将独立物品作为材料,会自动从材料中省略。
 * ＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
 * 用于替代尚未合成的物品名称:<Mask Name: x>
 * ※注意:需要插件参数"隐藏尚未合成的物品名称"为true。
 * 〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓
 * 疯狂模式-自定义合成效果
 * 道具/武器/防具备注:
 * ＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
 ...
 * 〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓
 * 插件命令:
 * 打开合成系统界面: OpenSynthesis
 * 主菜单显示/隐藏合成系统命令: ShowSynthesis / HideSynthesis
 * 主菜单启用/禁止合成系统命令: EnableSynthesis / DisableSynthesis
 * －－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
 * 仅打开指定配方: OpenSynthesis Item/Weapon/Armor 物品ID Recipe
 * ※注意:不需要提前获得配方。
 */
//=============================================================================
