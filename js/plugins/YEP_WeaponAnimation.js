 ...
 * ============================================================================
 *
 * 你是否希望给你的剑不同的图像尽管他们是一种类型的剑。或者是
 * 你的斧子？或者任何武器？现在你可以了。在这里，你可以自定义图像。
 *
 * 注意：如果你使用了YEP_BattleEngineCore,YEP_X_AnimatedSVEnemies
 * 或者其他动作序列，需要把这个插件放在他们后面。
 *
 * ============================================================================
 * Notetags
 * ============================================================================
 ...
 *
 *   <Weapon Image: x>
 *   用大于0的数代替x，然后你可以从你的img文件夹中获得图像。每一个完整
 *   部分包含12个武器图像。如果希望载入第一个部分的物品，他可能在1-12
 *   之间。如果你希望从第二部分载入，他可能在13-24之间等等。这个每次增长
 *   值为12，这意味着如果你想要武器从第50部分获得，x位于589到600之间。
 *
 *   默认，有下面的值参考:
 ...
 *   如果你创建了一个文件放置自定义武器部分，你可以使用这个标签来
 *   获得路径。这个名字不需要带有拓展名。例如你的物品部分叫做
 *   DaggerBlue.png，你可以使用这个命令
 *
 *   <Weapon Motion: thrust>
 *   <Weapon Motion: swing>
 *   <Weapon Motion: missile>
 *   当你使用自定义武器动画时，你可以这些来描述动作，你过没有设置
 *   ，动作将会采用插件中的默认值:
 ...
 * ============================================================================
 *
 * 这个武器部分格式将会遵从软件默认方式。但是，不像默认的武器部分
 * 格式，这些自定义图像需要被明确定义使其可以在默认战斗里面正确显示
 *
 * 这里有一个武器部分格式:
 *
