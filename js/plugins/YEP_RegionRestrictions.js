 ...
 * 注意：插件配置中定义的Region为全局生效，
 * 即在插件本身定义的region功能会适
 * 用于所有地图图层，如果你想在不同的地图定义不同的region阻挡功能，
 * 请参照下文的注释命令于各地图配置页面单独设置。
 * 不喜欢NPC进入他们不属于的区域？
 *
 * 不是所有人都想让NPC能够随便溜达的！
 * 本插件可以让你你的NPC无法穿越某Region图层覆盖的区域。
 * 同样的，你也可以使用region图层设置总是允许玩家、NPC穿越的
 * 区域，从而忽略地形限制，
 * 如此就不必担心过多的具有跟随功能的NPC把你逼近死胡
 ...
 *   阻止玩家 通过ID为x的region图层，可以单独定义1个ID，
 * 也可以定义复数个，还可
 *   以定义区间。以下同理
 *
