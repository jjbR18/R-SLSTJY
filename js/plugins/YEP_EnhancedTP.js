 ...
 *  ============================================================================
 * 
 * RPG-Maker-MV中的TP系统相当有限。很多TP系统
 * 硬编码使得RPG制作者MV用户很少控制TP的多少
 * 获得一个战斗者可以从特定的行动和情况中获得的奖励。这个
 * 插件让你能够调整多少TP战斗将获得
 * 各种动作，不同的TP模式，让玩家选择和挑选
 * 他们希望每个演员都有什么样的表演模式。
 * 
 *  ============================================================================
 * 说明-TP模式参数
 *  ============================================================================
 * 
 * TP模式是给你的演员，让他们决定如何获得他们想要的
 * 在整个战斗过程中。您可以决定每个TP模式的规则。
 * 
 * 下面概述了每个TP模式参数的作用：
 *
 *   Name
 ...
 * ============================================================================
 *
 * 您可以使用以下NoteTag来调整TP的各种设置。
 * 
 * 演员和敌人标签：
 * 
 *   <TP Mode: x>
 *   这将把参与者的默认TP模式设置为x。如果没有使用这个notetag
 *   那么actor将默认为插件参数中的模式。
 *
 *   <Unlock TP Mode: x>
 *   <Unlock TP Mode: x, x, x>
 *   <Unlock TP Mode: x to y>
 *   这将解锁参与者在默认情况下可以使用的TP模式。
 *   此标签将添加默认解锁时，插件的参数中已经预设了。
 *
 ...
 *   <Unlock TP Mode: x, x, x>
 *   <Unlock TP Mode: x to y>
 *   这将导致目标解锁TP模式x（至y）。这将使TP菜单中提供TP模式。
 *
 * Skill Notetags:
 * 
